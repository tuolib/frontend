...
- Step 1：登录页（含首次改密流程）
- Step 2：管理员管理（后台人员 CRUD，超管专属）
- Step 3：用户管理（C 端客户列表 + 详情 + 封禁/解封）
- Step 4：分类管理（树形表格、CRUD Modal、启用/禁用/删除）
- Step 5：商品列表（筛选、搜索、状态切换、删除）
- Step 6：商品创建（基本信息 + 分类选择）
- Step 7：商品编辑（信息编辑 + SKU 管理 + 图片管理）
- Step 8：订单列表（状态 Tabs、快捷发货）
- Step 9：订单详情（状态流程、商品明细、收货地址、支付记录、取消/退款）
- Step 10：Dashboard（统计卡片 + 近期订单 + 快捷操作）
- Step 11：库存调整（商品搜索 + SKU 选择 + 库存调整表单）
